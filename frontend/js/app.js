const App = {
  ...
  addNotification(type, title, amount, category) {
    ...
    if (enabled && 'Notification' in window && Notification.permission === 'granted') {
      ...
      new Notification('MoneyMint', { body, icon: 'data:image/svg+xml,<svg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 100 100%22><text y=%22.9em%22 font-size=%2290%22>\uD83C\uDF31</text></svg>' });
  ...
  setupHistoryListeners() {
    ...
    if (pdfBtn) {
      pdfBtn.addEventListener('click', () => {
        exportPDF('MoneyMint-Transactions');
      });
    }
    ...
    if (reportBtn) {
      reportBtn.addEventListener('click', () => {
        exportPDF('MoneyMint-Financial-Report');
      });
    }
  }
};
